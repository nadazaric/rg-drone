...
void main()
{
    ...
    vec3 viewDir = normalize(uLightPos - chFragPos); // TODO: Zar ovdje ne treba uViewPos - chFragPos jer trazimo ugao gledanja?
    vec3 reflectDir = reflect(-lightDir, norm); // reflektovani vektor svjetlosti u odnosu na normalu povrsine
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32); // skalarno mnozenje reflektovanog i ugla gledanja, n = 32  (kolicina bjelila koja se pojavljuje) - manji broj bjelje, manja kuglica ali ostrija i obrnuto
    ...
    for(int i = 0; i < 6; i++) {
        ...
        // vec3 ambient = ambientStrength * uLightColor;
        // TODO: Ambijentalna samo jednom, zar ne?
        ...
        vec3 viewDir = normalize(uViewPos - chFragPos); // TODO: Zar ovdje dejeluje okej
